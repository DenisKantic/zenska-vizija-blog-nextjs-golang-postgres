...
import { SaveEventToDB } from '../../../../actions/saveEventData'
...
const CreatePost = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formElement) {
      ...
      const result = await SaveEventToDB(formData, text)
  ...
  return (
    <div
      className="w-full h-full overflow-y-scroll text-black 
                    xxs:p-1 md:p-10"
    >
      <h1 className="text-4xl">Kreiraj događaj</h1>

      <form
        onSubmit={handleSubmit}
        className="flex flex-col justify-start items-start mt-10 min-h-screen"
      >
        <p className="text-xl mt-5">Naslov događaja</p>
        <input
          type="text"
          name="eventTitle"
        ...
        <p className="text-xl mt-5">Lokacija</p>
        <input
          type="text"
          name="location"
          placeholder="Unesite Vašu lokaciju"
          className="mt-5 text-[#C86DD7] bg-white text-xl rounded-full outline-none
          hover:outline-1 hover:outline-[#F93EDF] focus:outline-[#AC009B]
          xxs:text-sm xxs:p-2 xxs:w-full sm:p-7 sm:text-xl sm:py-3 lg:w-[80%] xl:w-[50%]"
          onChange={(e) => setTitle(e.target.value)}
        ...
        <p className="text-xl mt-5">Unesite vrijeme</p>
        <input
          type="text"
          name="time"
          placeholder="Unesite Vaše vrijeme"
          className="mt-5 text-[#C86DD7] bg-white text-xl rounded-full outline-none
          hover:outline-1 hover:outline-[#F93EDF] focus:outline-[#AC009B]
          xxs:text-sm xxs:p-2 xxs:w-full sm:p-7 sm:text-xl sm:py-3 lg:w-[80%] xl:w-[50%]"
          onChange={(e) => setTitle(e.target.value)}
