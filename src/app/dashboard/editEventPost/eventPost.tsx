...
export default function EventPost({ id }: any) {
  ...
  const handleUploadImage = async () => {
    try {
      if (image) {
        ...
        const submit = await editPostData(
          title,
          text,
          option,
          formattedDate,
  ...
  const editPostData = async (
    ...
    const docRef = doc(db, 'event', String(id))
  ...
  useEffect(() => {
    const getBlogData = async () => {
      try {
        const docRef = doc(db, 'event', String(id)) // Assuming params.blogId holds the ID of the blog post
